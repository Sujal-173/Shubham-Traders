  ...
  return (
    <article className="section bg-white">
      <div className="container max-w-3xl">
        <p className="mb-3 font-black uppercase tracking-[0.18em] text-solar">
          {post.category}
        </p>
        ...
        {Array.isArray(post.body) && post.body.length > 0 ? (
          ...
          <div className="mt-8 grid gap-4 text-lg leading-8 text-slate-700">
            <p>
              This article is CMS-ready. Add rich text in Sanity Studio to
              replace this placeholder with detailed SEO content, internal
              links, FAQs and lead CTAs.
            </p>

            <p>
              For now, it gives the website a complete blog architecture and
...
async function getPost(slug: string): Promise<Post | null> {
  try {
    const post = await client.fetch<CmsPost | null>(
      POST_QUERY,
      { slug },
      { next: { revalidate: 60 } }
    );
    ...
    if (post) {
      return {
        ...
        excerpt:
          post.excerpt ||
          "Read the latest solar guidance from Shubham Traders.",
